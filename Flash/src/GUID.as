package {
	...
	public class GUID extends Object {
		...
		public static function create():String {
			...
			var finalString:String = rawID.substring(0, 4) + "-" + rawID.substring(4,8) + "-" + rawID.substring(8, 12) + "-" + rawID.substring(12, 16) + "-" + rawID.substring(16, 20) + "-" + rawID.substring(20, 24) + "-" + rawID.substring(24, 28) + "-" + rawID.substring(28, 32);
			return finalString;
		}
		
		public static function getConcatID(string:String):String
		{
			var data:Array = string.split("-");
			var endString:String = "";
			var maxlen:int = data.length <= 6 ? data.length : 6;
			for (var i:int = 0; i < maxlen; i++)
			{
				endString += String(data[i]).substr(0, 1);
			}
