...
package com.worlize.gif
{
	...
	public class GIFPlayer extends Bitmap
	{
		...
		public function GIFPlayer(autoPlay:Boolean = true, callback:Function = null)
		{
			...
			urlLoader.addEventListener(Event.COMPLETE, loadedInData);
			//urlLoader.addEventListener(HTTPStatusEvent.HTTP_RESPONSE_STATUS, onHTTPSStatus);
			urlLoader.addEventListener(HTTPStatusEvent.HTTP_STATUS, onHTTPSStatus);
			urlLoader.addEventListener(IOErrorEvent.IO_ERROR, onIOError);
			_callback = callback;
		}
		
		protected function onIOError(error:IOErrorEvent):void
		{
			//Configuration.chatModule.stateMessage(error.toString());
			throw(error);
		}
		
		protected function onHTTPSStatus(error:HTTPStatusEvent):void
		{
		...
		protected function loadedInData(evt:Event):void
		{
			var myByteArray:ByteArray = urlLoader.data as ByteArray;
			loadBytes(myByteArray);
		}
...
//			stage.frameRate = 60;
		...
		private var _overrideLoopCount:int = -1;
		public function setLoopCount(count:uint):void
		{
			_overrideLoopCount = count;
		}
...
//			trace("Decode complete.  Total decoding time: " + gifDecoder.totalDecodeTime + " Blocking decode time: " + gifDecoder.blockingDecodeTime);
			currentLoop = 0;
			_loopCount = _overrideLoopCount != -1 ? _overrideLoopCount : gifDecoder.loopCount;
			_frames = gifDecoder.frames;
			_frameCount = _frames.length;
			_currentFrame = -1;
			_imageWidth = gifDecoder.width;
			_imageHeight = gifDecoder.height;
			if (_callback != null) _callback(_imageWidth, _imageHeight);
			gifDecoder.cleanup();
			unloadDecoder();
			setReady(true);
